...
public class RaycastAttack : NetworkBehaviour {
    ...
    [SerializeField] int gainPointsPerHit = 1;
    ...
    void Update() {
        ...
        if (attack.WasPerformedThisFrame()) {
            ...
            if (Runner.GetPhysicsScene().Raycast(ray.origin, ray.direction * shootDistance, out var hit)) {
                ...
                {
                    if (hitObject.gameObject.CompareTag(gameObject.tag))
                        return;
                    Debug.Log("Dealing damage");
                    health.DealDamageRpc(Damage);
                    gameObject.TryGetComponent<Points>(out var points);
                    points.GetPointsRpc(gainPointsPerHit);
                }
            }
        }
    }
